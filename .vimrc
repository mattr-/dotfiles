...
function! AlternateForCurrentFile()
  ...
  let in_app = match(current_file, '\<controllers\>') != -1 || match(current_file, '\<models\>') != -1 || match(current_file, '\<views\>') != -1 || match(current_file, '\<helpers\>' || match(current_file, '\<services\>') != -1
  if going_to_spec
    if in_app
